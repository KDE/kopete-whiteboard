...
WbScene::WbScene(const QString &session, const QString &ownJid, const QSize &s, QObject * parent) : QGraphicsScene(parent) {
	importing = false;
	highestIndex_ = 0;
	highestId_ = 0;
	session_ = session;
...
//        int a = s.width();
        QString width = "";
        width.append(QString("%1").arg(s.width()));
        QString height = "";
        height.append(QString("%1").arg(s.height()));
	// Default values
...
void WbScene::queueNew(const QString &id, const qreal &index, const QDomElement &svg) {
  qDebug() << "queueNew: " << svg.text();
	...
	n.appendChild(svg.cloneNode(true));//<<<<<<<<<<<<<<<<<<<<< SEGMENTATION FAULT
